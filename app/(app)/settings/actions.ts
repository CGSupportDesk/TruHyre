...
function fallbackAppHost() {
  return (process.env.NEXT_PUBLIC_APP_URL || "localhost:3000").replace(/^https?:\/\//, "").replace(/\/$/, "");
}
...
export async function createInvitationAction(formData: FormData): Promise<void> {
  ...
  const host = h.get("x-forwarded-host") || h.get("host") || fallbackAppHost();

...
const LEGACY_COMPANY_TAGLINE = process.env.LEGACY_COMPANY_TAGLINE;
const LEGACY_CLIENT_NAME = process.env.LEGACY_CLIENT_NAME;
const LEGACY_CLIENT_CONTACT = process.env.LEGACY_CLIENT_CONTACT;
...
async function main() {
  ...
  if (LEGACY_COMPANY_TAGLINE) {
    await db
      .update(companyProfile)
      .set({ tagline: "An internal hiring platform" })
      .where(eq(companyProfile.tagline, LEGACY_COMPANY_TAGLINE));
  }
  ...
  if (LEGACY_CLIENT_NAME) {
    await db
      ...
      .where(eq(clientAccounts.name, LEGACY_CLIENT_NAME));
  }
  if (LEGACY_CLIENT_CONTACT) {
    await db
      .update(clientContacts)
      .set({ name: "Demo Hiring Manager", phone: "+1 555 0100" })
      .where(eq(clientContacts.name, LEGACY_CLIENT_CONTACT));
  }
  if (LEGACY_USER_FULL_NAME) {
    await db
      .update(users)
      .set({ fullName: "Demo Hiring Mgr" })
      .where(eq(users.fullName, LEGACY_USER_FULL_NAME));
  }
